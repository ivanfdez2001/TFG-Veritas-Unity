...
public class QuizManagerNivel8 : MonoBehaviour
{
    ...
    void SetAnswers()
    {
        ...
        {
            options[i].GetComponent<AnswerScriptLevel8>().isCorrect = false;
            options[i].transform.GetChild(0).GetComponent<Text>().text = QnA[currentQuestion].Answers[i];

            if (QnA[currentQuestion].CorrectAnswer == i + 1)
            {
                options[i].GetComponent<AnswerScriptLevel8>().isCorrect = true;
            }
            options[i].GetComponent<Image>().color = options[i].GetComponent<AnswerScriptLevel8>().startColor;
        }
    }
